// page.tsx (blog)
// Developer: Marcus Daley
// Date: 2026-04-03
// Purpose: Dev Log page styled as mission log entries with status badges,
//          active mission panel, and command channel subscription section.

export default function Blog() {
  const blogPosts = [
    {
      id: 1,
      title: "Island Escape - Team Voxel Survival Game",
      date: "January 2026",
      excerpt: "Our team project featuring procedural terrain generation, AI combat systems, and comprehensive tool programming. Built with Unreal Engine 5.5.",
      link: "https://theendisnear469.itch.io/island-escape",
      tags: ["Unreal Engine", "C++", "Tool Programming", "Team Project"],
      type: "external"
    },
    {
      id: 2,
      title: "Building MCP Integration for Unreal Engine",
      date: "December 2025",
      excerpt: "Developing a custom Model Context Protocol server that enables AI-assisted game development workflows directly in Unreal Engine.",
      tags: ["Python", "FastMCP", "Unreal Engine", "AI Tools"],
      type: "internal"
    },
    {
      id: 3,
      title: "From Submarines to Game Development",
      date: "November 2025",
      excerpt: "My journey transitioning from 9 years as a Navy submarine mechanic to pursuing a career in game development and tool programming.",
      tags: ["Career", "Navy Veteran", "Full Sail University"],
      type: "internal"
    }
  ];

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900">
      <div className="container mx-auto px-6 py-20">
        {/* Header */}
        <div className="text-center mb-16">
          <p className="font-mono text-cyan-400 text-sm tracking-widest uppercase mb-4">
            // mission_logs
          </p>
          <h1 className="text-5xl font-bold mb-6 bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">
            DEV LOG
          </h1>
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Development insights, project updates, and mission reports
          </p>
        </div>

        {/* Log Entries */}
        <div className="flex flex-col gap-6 max-w-4xl mx-auto">
          {blogPosts.map((post, index) => (
            <article
              key={post.id}
              className={`bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700/50 ${
                post.id === 1 ? "border-l-2 border-l-yellow-500" : "border-l-2 border-l-cyan-500"
              }`}
            >
              {/* Log Header */}
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  <span className="font-mono text-cyan-400 text-sm">
                    LOG-{String(index + 1).padStart(3, "0")}
                  </span>
                  <span className="font-mono text-gray-500 text-xs">
                    {post.date}
                  </span>
                </div>
                {post.id === 1 ? (
                  <span className="px-2 py-0.5 bg-green-500/20 text-green-400 text-xs font-mono rounded">
                    LIVE
                  </span>
                ) : (
                  <span className="px-2 py-0.5 bg-yellow-500/20 text-yellow-400 text-xs font-mono rounded">
                    INCOMING
                  </span>
                )}
              </div>

              {/* Title */}
              <h2 className="text-xl font-bold text-white mb-3">
                {post.title}
              </h2>

              {/* Excerpt */}
              <p className="text-gray-300 mb-4 leading-relaxed">
                {post.excerpt}
              </p>

              {/* Tags */}
              <div className="flex flex-wrap gap-2 mb-6">
                {post.tags.map((tag, tagIndex) => (
                  <span
                    key={tagIndex}
                    className="px-3 py-1 bg-cyan-500/10 text-cyan-300 rounded-full text-sm font-mono"
                  >
                    {tag}
                  </span>
                ))}
              </div>

              {/* Action Button */}
              {post.type === "external" && post.link ? (
                <a
                  href={post.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-5 py-2.5 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 font-mono text-sm rounded-lg border border-cyan-500/30 hover:border-cyan-500/50 transition-all duration-300"
                >
                  View on itch.io
                  <svg
                    className="w-4 h-4"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"
                    />
                  </svg>
                </a>
              ) : (
                <button
                  className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-700/50 text-gray-500 font-mono text-sm rounded-lg border border-slate-600/30 cursor-not-allowed"
                  disabled
                >
                  Coming Soon
                </button>
              )}
            </article>
          ))}
        </div>

        {/* Active Mission Panel */}
        <div className="max-w-4xl mx-auto mt-16">
          <div className="bg-gradient-to-br from-cyan-500/10 to-blue-600/10 border border-cyan-500/20 rounded-xl p-8">
            <p className="font-mono text-cyan-400 text-sm tracking-widest uppercase mb-4">
              // active_mission
            </p>
            <h3 className="text-2xl font-bold text-white mb-3">
              ACTIVE MISSION
            </h3>
            <h4 className="text-xl text-cyan-300 font-semibold mb-4">
              Island Escape - Featured Project
            </h4>
            <p className="text-gray-300 mb-6 leading-relaxed">
              A comprehensive team project showcasing my tool programming and gameplay engineering skills.
              Features include procedural terrain generation, AI combat systems, modular pickup/inventory
              system, and designer-controlled spawner systems built with delegates.
            </p>
            <div className="flex flex-wrap gap-3">
              <a
                href="https://theendisnear469.itch.io/island-escape"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 font-mono text-sm rounded-lg border border-cyan-500/30 hover:border-cyan-500/50 transition-all duration-300"
              >
                Play on itch.io
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
              </a>
              <a
                href="/projects"
                className="inline-flex items-center gap-2 px-6 py-3 bg-slate-700/50 hover:bg-slate-700 text-gray-300 font-mono text-sm rounded-lg border border-slate-600/30 hover:border-slate-600/50 transition-all duration-300"
              >
                View All Projects
              </a>
            </div>
          </div>
        </div>

        {/* Subscribe to Command Channel */}
        <div className="max-w-2xl mx-auto mt-16 text-center">
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-8 border border-slate-700/50">
            <p className="font-mono text-cyan-400 text-sm tracking-widest uppercase mb-4">
              // comms
            </p>
            <h3 className="text-2xl font-bold text-white mb-4">
              SUBSCRIBE TO COMMAND CHANNEL
            </h3>
            <p className="text-gray-300 mb-6">
              Follow my journey as I complete my degree and enter the game development industry.
              More blog posts and project updates coming soon!
            </p>
            <div className="flex flex-wrap gap-4 justify-center">
              <a
                href="https://www.linkedin.com/in/marcusdaley-gamedev"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 font-mono text-sm rounded-lg border border-cyan-500/30 hover:border-cyan-500/50 transition-all duration-300"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
                Follow on LinkedIn
              </a>
              <a
                href="https://github.com/GrizzwaldHouse"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 bg-slate-700/50 hover:bg-slate-700 text-gray-300 font-mono text-sm rounded-lg border border-slate-600/30 hover:border-slate-600/50 transition-all duration-300"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
                </svg>
                View GitHub
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
